...
if ( ! function_exists( 'restaurateur_pagination' ) ) :
    function restaurateur_pagination($pages = '', $range = 3)
    {
...
function restaurateur_pagination($pages = '', $range = 4)
{
     ...
     if(1 != $pages)
     {
         ...
         for ($i=1; $i <= $pages; $i++)
         {
             if (1 != $pages &&( !($i >= $paged+$range+1 || $i <= $paged-$range-1) || $pages <= $showitems ))
             {
                 echo ($paged == $i)? "<span style='padding: 2px' class=\"current\"> ".$i." </span>":"<a style='padding: 2px' href='".get_pagenum_link($i)."' class=\"inactive\">  ".$i."  </a>";
             }
         }
